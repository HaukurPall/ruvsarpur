...
vod_types_and_categories = (
    [  # Note that this corresponds to the top level list of pages available (Sjonvarp, Utvarp, Krakkaruv, Ungruv)
        (1, "tv"),
        (2, "krakkaruv"),
        (2, "ungruv"),
    ]
...
def download_file(url, local_filename, display_title, keeppartial=False):
    try:
        ...
        printProgress(
            completed_size, completed_size, prefix="Downloading:", suffix="Complete", barLength=25, color=False
        )
...
def __create_retry_session(retries=5):
    ...
    retry = Retry(total=retries, read=retries, connect=retries, backoff_factor=0.3, status_forcelist=(500, 502, 504))
    ...
    for url in PLAYLIST_URLS:
        for num in range(30):
            for letter in [
                ...
                "K",
                "A",
            ]:  # ,'A','B','C','D','E','F','G','H','I','J','L','N','O','P','Q','U','V','W','X','Y','Z']:
                url_formatted = url.format(
                    shown_year, shown_month, shown_day, QUALITY_BITRATE[video_quality]["code"], pid, letter, num
                )
                if "vod_url" in item:
                    if (
                        "vod_alt" in item
                    ):  # If this is an alternative download url then don't inject the QUALITY bitrate (as it is not supported)
                        url_formatted = "{0}{1}.mp4.m3u8".format(item["vod_url"], item["vod_dlcode"])
                    else:
                        url_formatted = "{0}{1}/{2}.mp4.m3u8".format(
                            item["vod_url"], QUALITY_BITRATE[video_quality]["code"], item["vod_dlcode"]
                        )
                ...
                try:
                    ...
                    # Perform the get
                    request = __create_retry_session().get(
                        url_formatted, stream=False, timeout=5, verify=False, headers=headers
                    )
                    ...
                    fragments = [
                        "{0}/{1}".format(url_path, line.strip())
                    ...
                    if len(fragments) == 1 and fragments[0].find(".m3u8?tlm=hls&streams") > 0:
                        # Perform the get
                        request = __create_retry_session().get(
                            fragments[0], stream=False, timeout=5, verify=False, headers=headers
                        )
                    ...
                    print(
                        "Error while discovering playlist for {1} from '{0}'".format(
                            url_formatted, color_title(display_title)
                        )
                    )
                    print(ex)
...
def download_m3u8_playlist_using_ffmpeg(
    ffmpegexec, playlist_url, playlist_fragments, local_filename, display_title, keeppartial, video_quality
):
    ...
    ret = subprocess.Popen(
        prog_args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, env=my_env
    )
    try:
        while True:
            try:
                ...
                if " Opening 'https://{0}".format(RUV_URL) in line:
                    completed_chunks += 1
                    printProgress(
                        min(completed_chunks, total_chunks),
                        total_chunks,
                        prefix="Downloading:",
                        suffix="Working ",
                        barLength=25,
...
def getShowTimes(days_back=0):
    ...
    url = "http://muninn.ruv.is/files/xml/ruv/{0}/{1}/".format(
        from_date.strftime("%Y-%m-%d"), today.strftime("%Y-%m-%d")
    )
...
def findffmpeg(path_to_ffmpeg_install=None, working_dir=None):
    ...
    # Throw an error
    raise ValueError(
        "Could not locate FFMPEG install, please use the --ffmpeg switch to specify the path to the ffmpeg executable on your system."
    )


# Regex to extract the necessary VOD data from the files url
RE_CAPTURE_VOD_URL = re.compile(
    r"(?P<urlprefix>.*tlm=hls&streams=20\d{2}\/\d{2}\/[0-3]\d\/)\d+kbps\/(?P<dlcode>\w+)\.mp4\.m3u8", re.IGNORECASE
)
RE_CAPTURE_VOD_URL_ALTERNATE = re.compile(
    r"(?P<urlprefix>.*tlm=hls&streams=(20\d{2}\/\d{2}\/[0-3]\d\/)?)(?P<dlcode>\w+)\.mp4\.m3u8", re.IGNORECASE
)
...
def getVodSchedule(panelType, categoryName):
    ...
    graphql_data = (
        '?operationName=getFrontpage&variables={"type":"'
        + categoryName
        + '","limit":600}&extensions={"persistedQuery":{"version":1,"sha256Hash":"f640a5b54c853d6a071f7c3b27bf8c056854ab3200c1d35b0a624eb203dfc501"}}'
    )
    ...
    if (
        not data
        or not "data" in data
        or not "Featured" in data["data"]
        or not "panels" in data["data"]["Featured"]
        or len(data["data"]["Featured"]["panels"]) <= 0
    ):
        return schedule
    ...
    print(
        "{0} | Total: {1} Series in {2}".format(color_title("Downloading VOD schedule"), total_programs, categoryName)
    )
    ...
    for panel in panels:
        for program in panel["programs"]:
            ...
            try:
                ...
                print(
                    "Unable to retrieve schedule for VOD program '{0}', no episodes will be available for download from this program.".format(
                        program["title"]
                    )
                )
                continue
...
def getVodSeriesSchedule(sid, data):
    ...
    for episode in prog["episodes"]:
        ...
        entry["desc"] = (
            episode["description"]
            if "duration" in episode and len(episode["description"]) > 10
            else prog["short_description"]
        )
        ...
        entry["ep_num"] = (
            ...
            else getGroup(RE_CAPTURE_VOD_EPNUM_FROM_TITLE, "ep_num", episode["title"])
        )
